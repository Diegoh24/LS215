/*
takes an argument that contains the data, parses it, thne produces 2 basic stats
- number of email messages found in the string
- the date range of the email addresses

email message string has the following characteristics

-multiple email messages separated by the delimiter string
each email message has 5 parts,
- sender
subject
date
recipient body
...
function toDateString(dateString) {
  ...
  let [year, month, day] = [date[2], date[0] - 1, date[1]]
  return new Date(year, month, day).toDateString();
}

function mailCount(emailData) {
  let emailMessages = emailData.split(/##\|\|##/);
  let messageCount = emailMessages.length;
  let dates = emailData.match(/\d{2}-\d{2}-\d{4}/g).sort();

  let [minDate, maxDate] = [dates[0], dates[dates.length - 1]];
  [minDate, maxDate] = [toDateString(minDate), toDateString(maxDate)]
